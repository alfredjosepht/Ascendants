// src/app/student-dashboard/messaging/page.tsx
'use client';
...
export default function MessagingPage() {
  ...
  useEffect(() => {
    try {
      ...
      const loggedInId = localStorage.getItem(LOGGED_IN_USER_ID_KEY) || initialStudentData[0].id;
      const user = allStudents.find((s: Student) => s.id === loggedInId) || null;
      setCurrentUser(user);

      // Combine alumni and other students into one list for searching
      const allOtherUsers: ConversationPartner[] = [
          ...allAlumni, 
          ...allStudents.filter((s: Student) => s.id !== loggedInId)
      ];
      setSearchableUsers(allOtherUsers);

      // Simulate existing conversations for the student
